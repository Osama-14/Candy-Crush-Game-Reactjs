...
const App = () => {
  ...
  const checkForColoumOfFour = () => {
    for (let i = 0; i <= 39; i++) {
      const colomnOfFour = [i, i +width, i + width * 2, i + width * 3]
      const decidedColor = currentColorArrangement[i]

      if (colomnOfFour.every(square => currentColorArrangement[square] === decidedColor)) {
        colomnOfFour.forEach(square => currentColorArrangement[square] = blank)
        return true
      }
    }
  }
  ...
  const checkForColoumOfThree = () => {
    for (let i = 0; i <= 47; i++) {
      const colomnOfThree = [i, i +width, i + width * 2]
      const decidedColor = currentColorArrangement[i]

      if (colomnOfThree.every(square => currentColorArrangement[square] === decidedColor)) {
        colomnOfThree.forEach(square => currentColorArrangement[square] = blank)
        return true
      }
    }
  }
  ...
  return (
    <div className="app">
      <div className="game">
        ...
        return  <img key={index} src={candyColors}  alt={candyColors} data-id={index} draggable={true} onDragStart={dragStart} onDragOver={(e :DragEvent<HTMLImageElement>) => e.preventDefault()}
